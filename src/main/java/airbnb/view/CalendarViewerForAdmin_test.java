...
public class CalendarViewerForAdmin_test {

    public static void run(List<ReservationDTO> inputList, HouseDTO houseDTO){
        ...
        for (int i = 0; i < monthList.size(); i++) {
            selectMonth(monthList.get(i), inputList, houseDTO);
            System.out.println();
        }

    }

    private static void inputDate(int month,List<ReservationDTO> inputList, HouseDTO houseDTO ){
        ...
        arr = returnArr(objectList,houseDTO.getBedroom(),month);
        view(month, list,arr,houseDTO);

    }



    private static void view(int monthValue, List<Date> list,int[] arr,HouseDTO houseDTO) {
...
//                    System.out.printf("%-5s", arrayList_2.get(j));
    ...
    public static void selectMonth(int val, List<ReservationDTO> reservationDTOList, HouseDTO houseAndFeeDTO) {
